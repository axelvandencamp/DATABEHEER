...
-- check op aantal te importeren records: SELECT * FROM marketing._m_temp_datavalidatie WHERE out_moved > 0 AND out_match_level = 'P' AND out_addr_in IN (1,2)
-- check op postcodes: SELECT DISTINCT out1.*, cc.zip FROM marketing._m_temp_datavalidatie out1 LEFT OUTER JOIN public.res_country_city cc ON cc.zip = out1.out_post_cd WHERE COALESCE(cc.zip,'_') = '_'
-------------------------------------------------------------
SELECT sq3.*
FROM
	...
	FROM 
	 --SELECT sq1.partner_id, max(sim_straat) sim_straat FROM
		...
		WHERE out1.out_moved > 0 AND out1.out_match_level = 'P' AND out1.out_addr_in IN (1,2)
...
SELECT * FROM res_partner_address_state WHERE id IN (2,3) -- = 7 -- 
-------------------------------------------------------------
